...
namespace PetiteParser.Analyzer.Actions {
    ...
    /// </summary>
    internal class RemoveDuplicateTerms : IAction {
        ...
        static private bool termsSame(Grammar.Term term1, Grammar.Term term2) {
            if (term1.Rules.Count != term2.Rules.Count) return false;

            List<Grammar.Rule> rules = term2.Rules.ToList();
            foreach (Grammar.Rule rule1 in term1.Rules) {

                bool found = false;
                foreach (Grammar.Rule rule2 in rules) {
        ...
        static private void removeDuplicate(Grammar.Grammar grammar, Grammar.Term term1, Grammar.Term term2) {
