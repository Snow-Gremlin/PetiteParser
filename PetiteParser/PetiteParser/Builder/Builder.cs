    ...
    public class Builder {
        ...
        public void DetermineStates() {
            ...
            while (changed.Count > 0) {
                ...
                NextStates(state).Foreach(changed.Add);
            }
        }
        ...
        private void determineNextStateFragment(State state, Fragment fragment, HashSet<State> changed) {
            ...
            log?.AddInfo("    Adding fragment to state {0}.", next.Number);
        ...
        public HashSet<State> NextStates(State state) {
            log?.AddInfo("Next States from state {0}.", state.Number);
            ...
            for (int i = 0; i < state.Fragments.Count; ++i)
                determineNextStateFragment(state, state.Fragments[i], changed);
            return changed;
        }
