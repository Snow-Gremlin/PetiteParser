...
public class Watcher {
    ...
    virtual public void SetToken(State? state, Token? token) =>
        this.Output?.WriteLine("SetToken(state:" + (state?.Name ?? "null") +
    ...
    virtual public void StepToNext(State? state, Rune c, Scanner.Location? loc, Transition? trans) =>
        this.Output?.WriteLine("Step(state:" + (state?.Name ?? "null") + ", rune:" + c + ", loc:" +
            ", target:" + (trans is null ? "-" : "(target:"+trans.Target.Name+", consume:"+trans.Consume)+")");
    ...
    virtual public void YieldAndRescan(int count, Token? token, bool consume) =>
        this.Output?.WriteLine("YieldAndRescan(retoken:" + count + ", token:" + (token?.Name ?? "null") +
            ", text:"+(token?.Text ?? "null") + ", loc:[" + (token?.Start?.ToString() ?? "-") + ".." +
            (token?.End?.ToString() ?? "-") + "], consume:"+consume+")");
    ...
    virtual public void PushToError(Token? token) =>
        this.Output?.WriteLine("PushToError(token:" + (token.ToString() ?? "null") + ")");
